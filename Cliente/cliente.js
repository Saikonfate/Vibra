document.addEventListener('DOMContentLoaded', () => {
    ...
    const sidebarToggleBtn = document.querySelector(".user-btn"); // Botão que controla a sidebar

    if (!sidebar) {
        console.error("Elemento da sidebar (#sidebar) NÃO encontrado!");
    } else {
        console.log("Elemento da sidebar (#sidebar) encontrado.", sidebar);
    }

    if (!overlay) {
        console.error("Elemento do overlay (#overlay) NÃO encontrado!");
    } else {
        console.log("Elemento do overlay (#overlay) encontrado.", overlay);
    }

    if (!sidebarToggleBtn) {
        console.error("Botão de toggle da sidebar (.user-btn) NÃO encontrado!");
    } else {
        console.log("Botão de toggle da sidebar (.user-btn) encontrado.", sidebarToggleBtn);
    }

    // Função para abrir/fechar a sidebar, chamada pelo onclick no HTML
    window.toggleSidebar = function() {
        console.log('toggleSidebar foi chamada.');
        if (sidebar) {
            const isVisible = sidebar.classList.toggle("visible");
            console.log('Sidebar "visible" class toggled. Agora é visível?', isVisible);
            if (sidebarToggleBtn) {
                sidebarToggleBtn.setAttribute('aria-expanded', isVisible.toString());
            } else {
                console.warn("sidebarToggleBtn não encontrado dentro de toggleSidebar para setAttribute.");
            }
            if (overlay) {
                if (isVisible) {
                    overlay.classList.add("visible");
                    console.log('Overlay "visible" class adicionada.');
                } else {
                    overlay.classList.remove("visible");
                    console.log('Overlay "visible" class removida.');
                }
            } else {
                 console.warn("Elemento overlay não encontrado dentro de toggleSidebar.");
            }
        } else {
            console.error("Elemento da sidebar não encontrado dentro de toggleSidebar.");
        }
    };

    // Adiciona o evento de clique ao botão da sidebar, se ele existir no DOM
    if (sidebarToggleBtn) {
        sidebarToggleBtn.addEventListener('click', function(event) {
            console.log('Botão .user-btn clicado (JS event listener).');
        });
    }

    // Função para fechar a sidebar
    function closeSidebar() {
        console.log('closeSidebar foi chamada.');
        if (sidebar && sidebar.classList.contains("visible")) {
            sidebar.classList.remove("visible");
            console.log('Sidebar "visible" class removida por closeSidebar.');
            if (sidebarToggleBtn) {
                sidebarToggleBtn.setAttribute('aria-expanded', 'false');
            }
            if (overlay) {
                overlay.classList.remove("visible");
                console.log('Overlay "visible" class removida por closeSidebar.');
            }
        }
    }

    // Fecha a sidebar ao clicar no overlay, se o overlay existir
    if (overlay) {
    ...
    // Fecha a sidebar ao clicar fora dela e fora do botão de toggle
    window.addEventListener("click", function (event) {
        if (sidebar && sidebar.classList.contains("visible") &&
            sidebarToggleBtn &&
            !sidebar.contains(event.target) &&
            !sidebarToggleBtn.contains(event.target)) {
            console.log("Clique fora da sidebar e do botão detectado.");
            closeSidebar();
        }
    });
